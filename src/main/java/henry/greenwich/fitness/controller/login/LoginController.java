...
public class LoginController {
     ...
     * userRoleService - interact with user's role's service
     ...
     */
    public LoginController(FacebookAccountService facebookAccountService, UserProfileService userProfileService, UserRoleService userRoleService, GoogleAccountService googleAccountService, RoleService roleService) {
    ...
    /**
     * @param response        - is used to pass to generate JWT function to generate JWT
    ...
    public void loginByFacebook(HttpServletResponse response, @RequestBody FacebookAccount facebookAccount) {
        try {
            // check facebook's account was inserted to the database or not
            FacebookAccount selectedFacebookAccount = this.facebookAccountService.findFacebookAccountByFacebookId(facebookAccount.getFacebookId());
            if (selectedFacebookAccount != null) {
                // check account is actived or not
                if (selectedFacebookAccount.getUserProfile().getStatus() != 1) {
                    this.sendErrorResponse(response);
                } else {
                    // return generated token
                    this.sendJWT(
                            response,
                            selectedFacebookAccount.getUserProfile().getFullName(),
                            selectedFacebookAccount.getUserProfile().getId(),
                            selectedFacebookAccount.getFacebookId()
                    );
                }
                return;
            }
            ...
            // add facebook's account to the database
            ...
            if (insertedFacebookAccount != null) {
                ...
                if (role != null) {
                    ...
                    this.sendJWT(
                            response,
                            insertedFacebookAccount.getUserProfile().getFullName(),
                            insertedFacebookAccount.getUserProfile().getId(),
                            insertedFacebookAccount.getFacebookId()
                    );
                }
            }
        } catch (Exception e) {
    ...
    public void loginByGoogle(HttpServletResponse response, @RequestBody GoogleAccount googleAccount) {
        try {
            ...
            GoogleAccount selectedGoogleAccount = this.googleAccountService.findGoogleAccountByGoogleId(googleAccount.getGoogleId());
            if (selectedGoogleAccount != null) {
                ...
                if (selectedGoogleAccount.getUserProfile().getStatus() != 1) {
                    ...
                    this.sendJWT(
                            response,
                            selectedGoogleAccount.getUserProfile().getFullName(),
                            selectedGoogleAccount.getUserProfile().getId(),
                            selectedGoogleAccount.getGoogleId()
                    );
                }
                return;
            }
            ...
            if (insertedGoogleAccount != null) {
                ...
                if (role != null) {
                    ...
                    this.sendJWT(
                            response,
                            insertedGoogleAccount.getUserProfile().getFullName(),
                            insertedGoogleAccount.getUserProfile().getId(),
                            insertedGoogleAccount.getGoogleId()
                    );
                }
     ...
     * @param userName      - userName of user (is used to generated JWT and send to client)
    ...
    private void sendJWT(HttpServletResponse response, String userName, Long userProfileId, String socialId) {
        try {
            ...
            StringBuilder responseData = new StringBuilder("{\"userName\": \"" + userName + "\", \"token\": \"" + Constants.TOKEN_PREFIX + token + "\", \"roles\": [");
    ...
    /**
     * @param socialId - use socialId (facebook's id or google's id to generate token)
     * @return emailConfirmationToken
     */
    private String generateJWT(String socialId) {
        return JWT.create()
                .withSubject(socialId)
